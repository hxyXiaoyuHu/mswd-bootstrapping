""" Usage example: python main_diff_dimensions.py --model mean-decay --sample_dim 500 """
"""
    implement the simulations under different dimensions
...
if __name__ == '__main__':
    ...
    if model == 'joint':
        ...
        signal = torch.ones(5)
    elif model == 'var-decay':
        signal = torch.ones(1)*6.4
    else:
        signal = torch.ones(1)*0.64
    ...
    for mcrun in range(nrun):
        ...
        if opts.methods['pwdPerm'] == True:
            ...
            print('run {}, percentage of rejection: pwd perm k3 {} \
            '.format(mcrun, pwd_perm_rejPerc))         
